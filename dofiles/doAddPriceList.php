...
if(empty($membership) || empty($drugsandmedicines) || empty($supplies) || empty($laboratory) || empty($xray) || empty($ultrasound) || empty($ecg) || empty($oxygen) || empty($accomsubs) || empty($professionalfee) || empty($orfeedrfee)){
	...
	if(membershipCheck($membership)===true){
		$errors[] = "That membership type already exists.";
	}
}
if (!empty($errors)){
...
addPriceList($membership, $drugsandmedicines, $supplies, $laboratory, $xray, $ultrasound, $ecg, $oxygen, $accomsubs, $professionalfee, $orfeedrfee);
$return['error'] = false;
$return['msg'] = "Success";
}
