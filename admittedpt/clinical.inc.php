...
$c_day = strtotime(date('Y-m-d', time()));
$addate = strtotime($getdatead);

$no_of_hos = date('j', $c_day - $addate);
?>
			...
			<div class="sheet-holder">
				...
				<div class="clinical-row">
					...
					<div class="contentCell clinical-cont-serdeptatt">
						<?php
						$doc = getDoctorsTask($rid, $pid, "Attending");
						foreach ($doc as $d){
							$did = $d["did"];
			...
			<div class="sheet-holder">
				...
				<div class="clinical-row">
					<div class="contentCell clinical-cont-adphysician">
						<?php
						$doc = getDoctorsTask($rid, $pid, "Examined");
						if(!empty($doc)){
							foreach ($doc as $d){
								$did = $d["did"];
								$docname = "Dr. ".$d["firstname"]." ".$d["lastname"].", ".$d["title"];
							...
							$docex = getDoctorsTask($rid, $pid, "Attending");
							foreach ($docex as $d){
								$did = $d["did"];
								$docname = "Dr. ".$d["firstname"]." ".$d["lastname"].", ".$d["title"];
								$specialty = $d["specialization"];
						...
						?>
						<input type="text" name="adphysician" id="adphysician" class="c-text" style="width: 240px;" value="<?php echo $docname;?>"/>
					</div>
